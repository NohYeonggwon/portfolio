...
import { ActiveTabType, ProjectType } from "@/src/type/project";
  ...
  return (
    <>
      ...
      >
        {/* 컨텐츠 영역 */}
        <div
          className="max-w-187.5 h-full mx-auto bg-black/90"
          ...
          <div className="w-full h-full p-8 space-y-12 text-white overflow-y-auto">
            {activeTab === "Design" ? (
              <>
                {project.designImg ? (
                  <div>
                    {project.designImg.map((img, index) => (
                      <Image
                        key={index}
                        className="w-full"
                        src={img}
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.techStack.map((tech, index) => (
                          <div
                            key={index}
                            className="w-8 h-8 p-1 bg-white rounded-md"
                          >
                            {tech}
                          </div>
                        ))}
                  ...
                  <div className="ml-2 space-y-8">
                    ...
                    <div className="space-y-1">
                      ...
                      <ul className="text-white/80 ml-6 space-y-1">
                        {project.detail?.contents.first.map(
                          (content, index) => (
                            <ProjectDetailItem key={index} content={content} />
                          ),
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="space-y-1">
                      <h5>2. 내가 담당한 역할</h5>
                      <ul className="text-white/80 ml-6 space-y-1">
                        {project.detail?.contents.second.map(
                          (content, index) => (
                            <ProjectDetailItem key={index} content={content} />
                          ),
                        )}
                    ...
                    <div className="space-y-1">
                      <h5>3. 기술적 구현 포인트</h5>
                      <ul className="text-white/80 ml-6 space-y-1">
                        {project.detail?.contents.third.map(
                          (content, index) => (
                            <ProjectDetailItem key={index} content={content} />
                          ),
                        )}
                    ...
                    <div className="space-y-1">
                      <h5>5. 배운 점 & 개선 방향</h5>
                      <ul className="text-white/80 ml-6 space-y-1">
                        {project.detail?.contents.fifth.map(
                          (content, index) => (
                            <ProjectDetailItem key={index} content={content} />
                          ),
                        )}
